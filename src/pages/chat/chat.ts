...
export class ChatPage {
  ...
  constructor(private storage: Storage, public navCtrl: NavController, public navParams: NavParams, public http: HttpClient) {
    ...
    this.data.nick = '';
    this.data.timestamp = 1641042061;

    console.log('Hello RestServiceProvider Provider');

    Observable.interval(3000)
      .mergeMap(()    =>  http.get('http://dev-staging.thewellcloud.cloud/chat/' + this.timeStamp))
      .subscribe(data => {
        var nextTimeStamp = this.timeStamp;  // Determine the latest message received so we can just ask for latest
  ...
  sendMessage() {
    ...
    this.http.put('http://dev-staging.thewellcloud.cloud/chat/', this.data, { headers: headers }).subscribe(data => {
    }, err => {
      console.log(err);
    });
  ...
  function generateNick() {
    var first = nickDictionary[Math.floor(Math.random() * (nickDictionary.length - 1))]
    var second = nickDictionary[Math.floor(Math.random() * (nickDictionary.length - 1))]
    var suffix = Math.floor(Math.random() * 98 + 10)
